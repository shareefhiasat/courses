...
if docker-compose -f scripts/docker/docker-compose.dev.yml exec -T app-db pg_isready -U military_lms -d military_lms >/dev/null 2>&1; then
...
echo "   Access PostgreSQL:   docker exec courses-app-db psql -U military_lms -d military_lms"
...
echo "   - Database operations use PostgreSQL via Prisma"
