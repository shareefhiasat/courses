...
OUTPUT_FILE="/tmp/restore-data-v3.sql"
...
# Function to convert COPY to INSERT with ON CONFLICT
convert_copy_to_insert() {
    ...
    awk "/^COPY public\\.$table /,/^\\\\.$/" "$BACKUP_FILE" | \
    # Skip the COPY header and the \. terminator
    sed '1d;$d' | \
    # Convert tabs to proper SQL values
    while IFS=$'\t' read -ra values; do
        if [ ${#values[@]} -gt 0 ]; then
            # Build INSERT statement
            echo "INSERT INTO public.$table ($columns) VALUES ("
            for i in "${!values[@]}"; do
                val="${values[$i]}"
                if [ "$val" = "\\N" ]; then
                    echo -n "NULL"
...
# Convert users (with column mapping for new fields)
echo "Converting users..."
awk '/^COPY public\.users /,/^\\.$/' "$BACKUP_FILE" | \
sed '1d;$d' | \
while IFS=$'\t' read -ra values; do
    if [ ${#values[@]} -gt 0 ]; then
        echo "INSERT INTO public.users (id, email, \"firstName\", \"lastName\", \"displayName\", \"isActive\", \"createdBy\", \"updatedBy\", \"createdAt\", \"updatedAt\", \"keycloakId\", \"realName\", sequence, \"studentNumber\", additionalImageUrl, militaryIdImageUrl, profileImageUrl, qidImageUrl) VALUES ("
        for i in "${!values[@]}"; do
            val="${values[$i]}"
            if [ "$val" = "\\N" ]; then
                echo -n "NULL"
        ...
        # Add NULL for new image fields
        echo ", NULL, NULL, NULL, NULL) ON CONFLICT (id) DO NOTHING;"
    fi
done >> "$OUTPUT_FILE"
...
while IFS=$'\t' read -ra values; do
    if [ ${#values[@]} -gt 0 ]; then
        ...
        # Add NULL for new flexible scheduling fields
