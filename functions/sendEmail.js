...
exports.sendEmail = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (!configDoc.exists) {
      throw new functions.https.HttpsError('failed-precondition', 'SMTP configuration not found. Please configure email settings in the dashboard.');
    }
    ...
    if (!smtpConfig.host || !smtpConfig.port || !smtpConfig.user || !smtpConfig.password) {
      throw new functions.https.HttpsError('failed-precondition', 'Incomplete SMTP configuration');
    }
    ...
    if (data.bcc) {
      mailOptions.bcc = Array.isArray(data.bcc) ? data.bcc.join(', ') : data.bcc;
    }
    ...
    if (error.code === 'EAUTH') {
      throw new functions.https.HttpsError('failed-precondition', 'Email authentication failed. Please check SMTP credentials.');
    } else if (error.code === 'ECONNECTION') {
      throw new functions.https.HttpsError('unavailable', 'Could not connect to email server. Please check SMTP settings.');
    } else if (error.responseCode === 550) {
      throw new functions.https.HttpsError('invalid-argument', 'Invalid recipient email address.');
    } else {
...
exports.testEmail = functions.https.onCall(async (data, context) => {
  ...
  if (!userDoc.exists || userDoc.data().role !== 'admin') {
    throw new functions.https.HttpsError('permission-denied', 'Only admins can test email configuration');
  }
