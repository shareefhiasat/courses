...
(async function(){
  ...
  function switchTab(tab) {
    ...
    switch (tab) {
      ...
      default:
        container.innerHTML = `<h2>${tab.charAt(0).toUpperCase() + tab.slice(1)}</h2><p>Content for ${tab} tab.</p>`;
        break;
    }
  }
  ...
  function switchTab(tab) {
    ...
    if (tab === 'activities') loadActivitiesEditor();
  }
  
  // Allowlist management
  let allowlistCache = { allowedEmails: [], adminEmails: [] };

  async function loadAllowlist() {
    try {
      const config = await getAllowlistConfig();
      if (config.success && config.data) {
        allowlistCache.allowedEmails = config.data.allowedEmails || [];
        allowlistCache.adminEmails = config.data.adminEmails || [];
      } else {
        allowlistCache = { allowedEmails: [], adminEmails: [] };
      }
      renderAllowlist();
    } catch (e) {
      document.getElementById('allowlist-msg').textContent = 'Failed to load allowlist';
    }
  }
  ...
  function renderAllowlist() {
    ...
    const searchTerm = document.getElementById('allowlist-search')?.value.toLowerCase() || '';
    const roleFilter = document.getElementById('allowlist-role-filter')?.value || 'all';

    tbody.innerHTML = '';

    const combined = [
      ...allowlistCache.adminEmails.map(email => ({ email, role: 'admin' })),
      ...allowlistCache.allowedEmails.map(email => ({ email, role: 'allowed' }))
    ];

    const filtered = combined.filter(item => {
      const matchesSearch = item.email.toLowerCase().includes(searchTerm);
    ...
    filtered.forEach(item => {
      ...
      tr.innerHTML = `
        <td>${item.email}</td>
        <td><span class="badge" style="background:${item.role === 'admin' ? '#ffeeba' : '#eef'}">${item.role}</span></td>
        <td>
          <button class="btn-danger btn-sm" data-action="remove-allowlist" data-email="${item.email}" data-role="${item.role}">Remove</button>
        </td>
      `;
    ...
    if (filtered.length === 0) {
      tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:20px;opacity:0.6;">No matches found</td></tr>';
    }
  }

  // Handle adding new emails
  document.getElementById('add-allowed-btn')?.addEventListener('click', () => {
    const input = document.getElementById('add-allowed-input');
    const emails = input.value.split(/[,\n]/).map(e => e.trim().toLowerCase()).filter(e => e);
    if (emails.length === 0) return;
    
    allowlistCache.allowedEmails = [...new Set([...allowlistCache.allowedEmails, ...emails])];
    input.value = '';
    renderAllowlist();
    toast(`Added ${emails.length} allowed emails (don't forget to save)`, 'info');
  });
  ...
  document.getElementById('add-admin-btn')?.addEventListener('click', () => {
    ...
    allowlistCache.adminEmails = [...new Set([...allowlistCache.adminEmails, ...emails])];
    input.value = '';
    renderAllowlist();
    toast(`Added ${emails.length} admin emails (don't forget to save)`, 'info');
  });

  // Handle removing emails
  document.querySelector('#allowlist-table tbody')?.addEventListener('click', (e) => {
    const btn = e.target.closest('button[data-action="remove-allowlist"]');
    if (!btn) return;
    ...
    if (role === 'admin') {
      allowlistCache.adminEmails = allowlistCache.adminEmails.filter(e => e !== email);
    } else {
      allowlistCache.allowedEmails = allowlistCache.allowedEmails.filter(e => e !== email);
    }
    renderAllowlist();
  });

  // Search and Filter listeners
  document.getElementById('allowlist-search')?.addEventListener('input', renderAllowlist);
  document.getElementById('allowlist-role-filter')?.addEventListener('change', renderAllowlist);

  // Export functionality
  document.getElementById('export-allowlist')?.addEventListener('click', () => {
    const combined = [
      ...allowlistCache.adminEmails.map(email => ({ email, role: 'admin' })),
      ...allowlistCache.allowedEmails.map(email => ({ email, role: 'allowed' }))
    ];
    ...
    let csvContent = "data:text/csv;charset=utf-8,Email,Role\n" 
      + filtered.map(e => `${e.email},${e.role}`).join("\n");
  ...
  document.getElementById('save-allowlist').addEventListener('click', async () => {
    const res = await setAllowlistConfig({ 
      allowedEmails: allowlistCache.allowedEmails, 
      adminEmails: allowlistCache.adminEmails 
    });
    document.getElementById('allowlist-msg').textContent = res.success ? 'Saved successfully' : 'Failed to save: ' + res.error;
    toast(res.success ? 'Allowlist updated' : (res.error || 'Failed to update allowlist'), res.success ? 'success' : 'error');
  });
