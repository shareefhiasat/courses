...
(async function(){
  ...
  async function loadAllowlist() {
    const allowedTextarea = document.getElementById('allowed-emails');
    const adminTextarea = document.getElementById('admin-emails');
    
    try {
      const config = await getAllowlistConfig();
      if (config.success && config.data) {
        allowedTextarea.value = (config.data.allowedEmails || []).join('\n');
        adminTextarea.value = (config.data.adminEmails || []).join('\n');
      } else {
        // No config found, start with empty lists
  ...
  document.getElementById('save-allowlist').addEventListener('click', async () => {
    const allowedEmails = document.getElementById('allowed-emails').value.split('\n').map(e => e.trim()).filter(e => e);
