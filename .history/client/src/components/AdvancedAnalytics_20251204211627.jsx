...
export default function AdvancedAnalytics() {
  ...
  const processWidgetData = (widget, comparisonOffset = 0) => {
    ...
    // Special handling for quiz analytics
    if (dataSource === 'quizzes') {
      if (groupBy === 'difficulty') {
        dataset.forEach(quiz => {
          const difficulty = quiz.difficulty || 'general';
        ...
        dataset.forEach(quiz => {
          const classId = quiz.classId || quiz.class || 'general';
          grouped[classId] = (grouped[classId] || 0) + 1;
        });
      } else {
      ...
      // Quiz submission analytics with performance ratio
      dataset.forEach(submission => {
        const key = submission[groupBy] || 'Unknown';
        const score = parseFloat(submission.score || submission.percentage || 0);
        const maxScore = parseFloat(submission.maxScore || 100);
        const performanceRatio = maxScore > 0 ? (score / maxScore) * 100 : 0;
        
        if (aggregation === 'count') {
          grouped[key] = (grouped[key] || 0) + 1;
        } else if (aggregation === 'avg') {
          if (!grouped[key]) grouped[key] = { sum: 0, count: 0 };
          grouped[key].sum += performanceRatio;
          grouped[key].count += 1;
        } else if (aggregation === 'sum') {
          grouped[key] = (grouped[key] || 0) + performanceRatio;
        } else {
          grouped[key] = (grouped[key] || 0) + 1;
        }
      });
      ...
      if (groupBy === 'status' || groupBy === 'attendanceStatus') {
        ...
        dataset.forEach(record => {
          const classId = record.classId || record.class || 'general';
          if (!grouped[classId]) {
            grouped[classId] = {
              present: 0,
              late: 0,
          ...
          const status = record.status || 'unknown';
          if (status === 'present') grouped[classId].present++;
          else if (status === 'late') grouped[classId].late++;
          else if (status === 'absent_no_excuse' || status === 'absent') grouped[classId].absent_no_excuse++;
          else if (status === 'absent_with_excuse' || status === 'excused') grouped[classId].absent_with_excuse++;
          else if (status === 'excused_leave') grouped[classId].excused_leave++;
          else if (status === 'human_case') grouped[classId].human_case++;
          grouped[classId].total++;
        });
      } else {
        dataset.forEach(item => {
          const key = item[groupBy] || 'Unknown';
    ...
    // Special handling for attendance by class (returns multiple series)
    if (dataSource === 'attendance' && (groupBy === 'classId' || groupBy === 'class')) {
      Object.entries(grouped).forEach(([label, stats]) => {
        if (typeof stats === 'object' && stats.total !== undefined) {
          // Return multiple series for attendance breakdown
          chartData.push(
            { label: `${label} - Present`, value: stats.present },
            { label: `${label} - Late`, value: stats.late },
            { label: `${label} - Absent (No Excuse)`, value: stats.absent_no_excuse },
            { label: `${label} - Absent (Excused)`, value: stats.absent_with_excuse },
            { label: `${label} - Excused Leave`, value: stats.excused_leave },
            { label: `${label} - Human Case`, value: stats.human_case }
          );
        } else {
          chartData.push({ label, value: stats });
        }
      });
    } else {
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              ...
              <div>
                ...
                >
                  ...
                  <option value="emailLogs">Email Logs</option>
                </select>
              </div>
