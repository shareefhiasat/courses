...
import { User, Mail, Phone, Hash, Palette, Save, Settings, Shield, Crown, Globe, Volume2, Vibrate, Smartphone, Bell } from 'lucide-react';
          ...
          <CardBody>
            ...
            <div className={styles.formSection}>
              ...
              {!notificationSettings.permissionsRequested && (
                <div className={styles.formGroup}>
                  <label>
                    ...
                    {t('notification_permissions') || 'Notification Permissions'}
                  </label>
                  <Button
                    variant="primary"
                    onClick={async () => {
              ...
              <div className={styles.formGroup}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <Button
                    variant={notificationSettings.soundEnabled ? 'primary' : 'outline'}
                    onClick={async () => {
                      const success = await updateSetting('soundEnabled', !notificationSettings.soundEnabled);
                      if (success) {
                        toast.success(notificationSettings.soundEnabled 
                    ...
                    className={styles.otpToggle}
                  >
                    {notificationSettings.soundEnabled ? (t('enabled') || 'Enabled') : (t('disabled') || 'Disabled')}
                  </Button>
                ...
                <p className={styles.helpText}>
                  {t('sound_description') || 'Play notification sounds for new messages and alerts.'}
                </p>
              </div>
              ...
              {checkSupport().vibration && (
                <div className={styles.formGroup}>
                  <label>
                    <Vibrate size={18} style={{ marginRight: '0.5rem' }} />
                    {t('notification_vibration') || 'Notification Vibration'}
                  </label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <Button
                      variant={notificationSettings.vibrationEnabled ? 'primary' : 'outline'}
                      onClick={async () => {
                        const success = await updateSetting('vibrationEnabled', !notificationSettings.vibrationEnabled);
                        if (success) {
                          toast.success(notificationSettings.vibrationEnabled 
                      ...
                      className={styles.otpToggle}
                    >
                      {notificationSettings.vibrationEnabled ? (t('enabled') || 'Enabled') : (t('disabled') || 'Disabled')}
                    </Button>
                    {notificationSettings.vibrationEnabled && (
                      ...
                      >
                        📳 {t('test_vibration') || 'Test'}
                      </Button>
                    )}
                  </div>
                  <p className={styles.helpText}>
                    {t('vibration_description') || 'Vibrate for notifications even when the screen is off.'}
                  </p>
                </div>
              )}
              ...
              {checkSupport().notification && (
                <div className={styles.formGroup}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <Button
                      variant={notificationSettings.browserNotificationsEnabled ? 'primary' : 'outline'}
                      onClick={async () => {
                        const success = await updateSetting('browserNotificationsEnabled', !notificationSettings.browserNotificationsEnabled);
                        if (success) {
                          toast.success(notificationSettings.browserNotificationsEnabled 
                      ...
                      className={styles.otpToggle}
                    >
                      {notificationSettings.browserNotificationsEnabled ? (t('enabled') || 'Enabled') : (t('disabled') || 'Disabled')}
                    </Button>
                  </div>
                  <p className={styles.helpText}>
                    {t('browser_notifications_description') || 'Show browser notifications when the page is in the background.'}
                  </p>
                </div>
              )}
              ...
              <div style={{ 
                ...
                <div style={{ fontWeight: 600, marginBottom: '0.5rem' }}>{t('supported_features') || 'Supported Features'}:</div>
                <div style={{ display: 'grid', gap: '0.25rem' }}>
                  <div>🔊 {t('audio_notifications') || 'Audio Notifications'}: {checkSupport().audio ? (t('supported') || 'Supported') : (t('not_supported') || 'Not Supported')}</div>
                  <div>📳 {t('vibration') || 'Vibration'}: {checkSupport().vibration ? (t('supported') || 'Supported') : (t('not_supported') || 'Not Supported')}</div>
                  <div>🔔 {t('browser_notifications') || 'Browser Notifications'}: {checkSupport().notification ? (t('supported') || 'Supported') : (t('not_supported') || 'Not Supported')}</div>
                  <div>📱 {t('mobile_device') || 'Mobile Device'}: {isMobile() ? (t('yes') || 'Yes') : (t('no') || 'No')}</div>
                </div>
              </div>
            </div>
          </CardBody>
