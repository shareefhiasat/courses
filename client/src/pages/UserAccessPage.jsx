...
const UserCategoryAccessPage = () => {
  ...
  const handleSaveAccess = async (e) => {
    ...
    try {
      ...
      // Validation
      if (!formData.userId || (typeof formData.userId === 'string' && formData.userId.trim() === '')) {
        toast.error('User is required');
        return;
      }
      if (!formData.categoryId || (typeof formData.categoryId === 'string' && formData.categoryId.trim() === '')) {
  ...
  const gridColumns = useMemo(() => {
    ...
    return columns;
  }, [auditColumns, handleEditAccess, deleteEntity, saving, t]);
    ...
    <div style={{ padding: '1.5rem' }}>
      ...
      <div style={{
        ...
        <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: '500' }}>
          User Category Access ({filteredAccesses.length}{filteredAccesses.length !== accesses.length ? ` of ${accesses.length}` : ''})
        </h3>
      </div>
