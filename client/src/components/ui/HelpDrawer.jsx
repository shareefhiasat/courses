...
const HelpDrawer = () => {
  ...
  useEffect(() => {
    // info('HelpDrawer - currentHelp changed or drawer opened:', {
    //   isOpen,
    ...
    if (currentHelp && isOpen) {
      // info('Resetting HelpDrawer state for new help content');
      setSearchTerm('');
      setExpandedSections({});
      ...
      // info('Setting new filtered content with', newContent.length, 'sections');
  ...
  useEffect(() => {
    // info('HelpDrawer - Search effect running', {
    //   isOpen,
    ...
    if (!isOpen) {
      // info('HelpDrawer - Drawer is closed, skipping filter');
      return;
    }
    
    if (!currentHelp?.content) {
      // info('HelpDrawer - No help content available');
      setFilteredContent([]);
      return;
    }
    ...
    if (!searchTerm.trim()) {
      // info('HelpDrawer - No search term, using full help content');
    ...
    const filterContent = () => {
      // info('Filtering content with searchTerm:', searchTerm);
  ...
  const renderContent = () => {
    try {
      ...
      error('[HelpDrawer] Error rendering help content:', error);
      return (
        <div style={{ 
          padding: '1rem',
