...
- Name: auth.spec.js >> Authentication — Login Flow >> TC-AUTH-010: Login page has remember me option (if configured)
- Location: tests/e2e/specs/auth.spec.js:139:3
...
> 140 |     await page.goto(`${BASE_URL}${DASHBOARD_ROUTE}`);
  ...
  227 |     ).first();
  228 |     await expect(logoutBtn).toBeVisible({ timeout: 10000 });
  229 |     await logoutBtn.click();
  230 |     await page.waitForTimeout(3000);
  231 |     const url = page.url();
  232 |     expect(url.includes('login') || url.includes('keycloak') || url.includes('8080')).toBe(true);
  233 |   });
  234 | 
  235 |   test('TC-AUTH-016: Logout from instructor role', async ({ page }) => {
  236 |     await gotoWithAuth(page, HOME_ROUTE, 'instructor');
  237 |     await dismissOverlays(page);
  238 |     const logoutBtn = page.locator(
  239 |       'button:has-text("Logout"), button:has-text("Sign Out"), ' +
  240 |       '[data-testid*="logout"]'
```