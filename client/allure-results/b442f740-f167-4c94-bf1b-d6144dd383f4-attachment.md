...
- Name: notifications-ui.spec.js >> Notifications UI — Unauthenticated >> TC-NOT-UI-028: No bell for unauthenticated
- Location: tests/e2e/specs/notifications-ui.spec.js:273:3
...
```ts
  174 |   });
  175 | 
  176 |   test('TC-NOT-UI-018: Click mark all as read', async ({ page }) => {
  177 |     const markAllBtn = page.locator('button:has-text("Mark All Read"), button:has-text("Mark all as read"), [data-testid*="mark-all"]').first();
  178 |     if (!(await markAllBtn.isVisible({ timeout: 3000 }).catch(() => false))) test.skip(true, 'No mark all read button');
  179 |     await markAllBtn.click();
  180 |     await page.waitForTimeout(2000);
  181 |     const hasContent = await waitForContent(page);
