...
- Name: auth.spec.js >> Authentication — Protected Route Enforcement >> TC-AUTH-029: Unauthenticated /dashboard redirects to login
- Location: tests/e2e/specs/auth.spec.js:417:3

# Error details

```
Error: page.goto: net::ERR_CONNECTION_TIMED_OUT at https://localhost:5174/dashboard
Call log:
...
```ts
  318 |   test('TC-AUTH-022: Session persists across navigation', async ({ page }) => {
  319 |     await gotoWithAuth(page, HOME_ROUTE, 'superAdmin');
  320 |     // Navigate to different pages
  321 |     await page.goto(`${BASE_URL}/profile`);
  322 |     await page.waitForLoadState('networkidle');
  323 |     await page.waitForTimeout(1000);
  324 |     const url = page.url();
  325 |     expect(url).not.toContain('/login');
  326 |     expect(url).not.toContain('keycloak');
  327 |   });
  328 | 
  329 |   test('TC-AUTH-023: Session warning modal can be triggered (dev mode)', async ({ page }) => {
  330 |     await gotoWithAuth(page, HOME_ROUTE, 'superAdmin');
  331 |     // In dev mode, window.testSessionWarning() triggers the modal
