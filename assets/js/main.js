...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- RENDER FUNCTIONS ---
  function render() {
    console.log("[App] Rendering UI with state:", {
  ...
  function applyLanguage() {
    ...
    const translations = {
      ...
      "main-subtitle": {
        en: "Interactive exercises and games for mastering programming concepts",
        ar: "تمارين وألعاب تفاعلية لإتقان مفاهيم البرمجة",
      },
    };
  ...
  function renderAuthBar() {
    ...
    if (state.currentUser) {
      ...
      const quickLinks = el(
        ...
        el(
          "a",
          { href: "dashboard.html", style: { textDecoration: "none" } },
          el(
            "button",
            {
              style: {
        ...
        state.allowlist.adminEmails
          ?.map((e) => e.toLowerCase())
          .includes(state.currentUser.email?.toLowerCase() || "")
          ? el(
              "span",
              {
                style: {
      ...
      const avatar = el(
        ...
        (state.currentUser.displayName || state.currentUser.email || "U")
          .charAt(0)
          .toUpperCase()
      );
      ...
      // Menu entries
      const linksInMenu = el(
        "div",
        { style: { padding: "6px 12px", borderBottom: "1px solid #eee" } },
        el(
          "a",
          {
            href: "admin.html",
            style: {
              display: state.allowlist.adminEmails
                ?.map((e) => e.toLowerCase())
                .includes(state.currentUser.email?.toLowerCase() || "")
                ? "inline-block"
                : "none",
      ...
      const toggleWrap = el(
        ...
        el(
          "button",
          {
            style: btnSegStyle(() => state.authMode === "login"),
            onclick: () => {
        ...
        el(
          "button",
          {
            style: btnSegStyle(() => state.authMode === "signup"),
            onclick: () => {
  ...
  async function handleAuth(mode, email, password, confirmPassword) {
    ...
    try {
      ...
      if (res.success) {
        showToast(
          mode === "login"
            ? t("loggedIn", state.lang)
            : t("accountCreated", state.lang),
          "success"
        );
        setTimeout(() => {
        ...
        const err =
          typeof res.error === "string" ? res.error : "Authentication error";
        showToast(err, "error");
  ...
  async function init() {
      ...
      if (user) {
        try {
          ...
          if (progressRes.success)
            state.userProgress = progressRes.progress || {};
          // Fetch user data to get enrolled classes for announcements
          const userDoc = await getDoc(doc(db, "users", user.uid));
          const enrolledClasses = userDoc.exists()
            ? userDoc.data().enrolledClasses || []
            : [];
    ...
    window.__appDebug = {
      ...
      forceLoggedInUI: (email = "debug@example.com") => {
        state.currentUser = { email };
        render();
      },
    };
